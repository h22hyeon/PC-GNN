...
def test(test_nodes: np.array, labels: np.array, model, batch_size: int, 
         flag="valid", valid_thresh=None) -> Tuple[float, float, float, float]:
        ...
        if flag == 'valid':
            f1, threshold = get_best_f1(labels, output_list[2])
            preds = np.where(output_list[2] > threshold, 1, 0)
            f1_macro = f1_score(labels, preds, average='macro')
        else:
            preds = np.where(output_list[2] > valid_thresh, 1, 0)
        ...
        return auc, recall, f1_macro, precision, threshold

def get_best_f1(labels: np.array, probs: np.array) -> Tuple[float, float]:
    """
    Calculate the F1 with best threshold.
    
    Determine the best threshold of the confidence of predicted anomalies.
    Calculate the F1 score with the theshold.
    """
    # Initialize the F1 and threshold.
    best_f1, best_thresh = 0, 0
    for thresh in np.linspace(0.01, 0.99, 100): # Set the thresholds.
        binarized_preds = np.zeros_like(labels) 
        binarized_preds[probs > thresh] = 1 # Binarize the prediction the threshold.
        temp_f1 = f1_score(labels, binarized_preds) # Calculate the F1 score.
        if temp_f1 > best_f1: # Compare the F1 scores.
            best_f1 = temp_f1
            best_thresh = thresh

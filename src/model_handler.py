...
class ModelHandler(object):
	...
	def __init__(self, config):
		...
		if args.data_name != 'amazon':
			...
			idx_train, idx_rest, y_train, y_rest = train_test_split(index, labels[3305:], stratify=labels[3305:], train_size=args.train_ratio, random_state=args.seed, shuffle=True)
	...
	def train(self):
		...
		if args.model == 'PCGNN':
			if (args.data_name == "yelp") or (args.data_name == "amazon"):
		...
		f1_mac_best, auc_best, epoch_best = 0, 0, -1
		...
		for epoch in range(args.epochs):
			...
			if (epoch+1) % args.valid_epochs == 0:
				...
				auc_val, recall_val, f1_val, precision_val = test(idx_valid, y_valid, model, self.args.batch_size, self.result, epoch, epoch_best, flag="val")
				if auc_val > auc_best:
					gnn_recall_best, f1_mac_best, auc_best, epoch_best = recall_val, f1_val, auc_val, epoch
		...
		auc_test, recall_test, f1_test, precision_test = test(idx_test, y_test, model, self.args.batch_size, self.result, epoch_best=epoch_best, flag="test")
